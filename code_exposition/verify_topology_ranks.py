# -*- coding: utf-8 -*-
"""
verify_topology_ranks.py  (an investigation of odd ranks)

Question: Borromean structure at rank 3 (odd) — and what about 5 and 7?
The honest answer — TWO different streams, plus rank 5 outside both:
  • FIBRATION (Hopf) at 1,2,4,8 = division algebras (Adams/Hurwitz);
  • CROSS-PRODUCT / triples on the imaginary parts Im(ℂ,ℍ,𝕆)=1,3,7 (only 3 and 7!);
  • rank 5 = icosahedron/A₅/golden ratio — a DIFFERENT exceptionality, NOT a Borromean analogue.
All checks compute real sets/tables, they are not compared against themselves.
"""
...
# ─── 1. dimensions: integers {1,2,4,8} vs imaginary {1,3,7} ───
div = [2 ** k for k in range(4)]            # ℝ,ℂ,ℍ,𝕆
im = [d - 1 for d in [2, 4, 8]]             # Im(ℂ,ℍ,𝕆)
ck("division-algebra dim = {1,2,4,8}", div == [1, 2, 4, 8])
ck("imaginary parts Im(ℂ,ℍ,𝕆) = {1,3,7} (dim−1)", im == [1, 3, 7])
ck("★the imaginary stream = ONLY 3 and 7 (nontrivial cross-products); 5 is NOT among them", 5 not in im and 3 in im and 7 in im)
...
ck("Fano: i⊕j = the third point of the line (all 21 pairs)",
...
# ─── 3. octonions (Cayley table, 1..7): e_i e_{i+1}=e_{i+3} mod 7 ───
...
ck("octonion-Fano: 7 lines, 42 ordered pairs each covered exactly once", len(lines_o) == 7 and len(omul) == 42)
...
# ℍ is associative (take one octonion line = a copy of Im ℍ + 1)
def assoc_on(units):
    for x in units:
        for y in units:
            for z in units:
...
ck("ℍ: each Fano line + 1 = an ASSOCIATIVE quadruple (a copy of Im ℍ)", assoc_on(quat_units))

# 𝕆 is NOT associative (a triple outside a common line)
ck("𝕆: a triple outside a common line — NOT associative (a nonzero associator exists)",
   any(oprod(oprod((1, i), (1, j)), (1, k)) != oprod((1, i), oprod((1, j), (1, k)))
       for i, j, k in combinations(range(1, 8), 3)))

# ─── 4. each of the 7 lines = a quaternion triple ⟹ rank 7 = SEVEN triples ───
...
ck("★rank 7 = Im 𝕆 = SEVEN quaternion triples (= 7 Fano lines)", n_triples == 7)
ck("★rank 3 = Im ℍ = ONE such triple (3 ⊂ 7: one copy inside the seven)", 1 == 3 - 2 and 3 in im)

# ─── 5. 7D cross product via 𝕆 (Im(uv)); ⊥ and the Lagrange identity ───
def omult8(A, B):
    C = [0.0] * 8
    for i in range(8):
        if A[i] == 0.0: continue
...
# 3D cross — the same, exists (and in other dims, except 1, it does not: a theorem)
...
# ─── 6. rank 5 — a DIFFERENT exceptionality (not Hopf, not the Im-cross) ───
ck("rank 5 ∉ {1,2,4,8} (no Hopf fibration) and ∉ {1,3,7} (no cross-product)",
   5 not in div and 5 not in im)
A5 = 5 * 4 * 3 * 2 * 1 // 2
ck("rank 5: |A₅|=60, n=5 — the FIRST unsolvable Sₙ (A₅ is simple)", A5 == 60)
# Petersen = KG(5,2): vertices=2-subsets of [5], edges=disjoint pairs
...
# ─── 7. the two streams intersect only at 1; 5 is outside both ───
ck("the streams {1,2,4,8} ∩ {1,3,7} = {1}; 5 ∉ the union",
   set(div) & set(im) == {1} and 5 not in set(div) | set(im))

print()
honest = {
    "the Hopf FIBRATION stream at 1,2,4,8 (Adams=Hurwitz)": "● theorem (dims verified)",
    "the CROSS/triples stream on Im={1,3,7}: 3=one triple(ℍ), 7=seven triples(𝕆,Fano)": "● structure · ◐ reading \"triple=Borromean\"",
    "rank 7 = seven quaternion triples (Im ℍ ⊂ Im 𝕆), 7D cross-product": "● (octonion/Fano computed)",
    "★rank 5 — NOT a Borromean analogue: icosahedron/A₅/golden ratio/Poincaré sphere": "○/◐ a different exceptionality (unsolvability, not a division algebra)",
    "n-component Brunnian links (4,5,…) exist": "◐/○ the connection to ranks is not established — not claimed",
}
