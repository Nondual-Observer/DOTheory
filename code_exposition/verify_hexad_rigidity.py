...
# -*- coding: utf-8 -*-
"""
verify_hexad_rigidity.py — rigidity of the rank-3 hexad, four readings of rank, builder/guardian.

  A. Rigidity: of 720 permutations of U₃ all three relations R₁,R₂,R₃ preserve EXACTLY 12 (D₆);
     preserving one R₁ already entails R₂ and R₃ (Hamming distance = cyclic on C₆).
  B. Four readings of rank n: simplex n+1 / cross-polytope 2n / cube 2ⁿ / symmetry n!;
     cube and cross-polytope are dual (vertex/facet counts swap places), the simplex is self-dual.
  C. Two hexads: 2n=n! is solvable exactly at n=3 (scene 2n and symmetry n! coincide in number only there).
  D. Builder/guardian: a+b=a·b in the natural numbers exactly at a=b=2 (node 4, where the sides coincide).
Register: A–D — ● enumeration/arithmetic; readings "act/body/world/symmetry" and "builder/guardian" — ◐ (so marked in the chapter).
"""
...
def section_A():
    print("\n[A] Rigidity of the hexad: 12 of 720; R₁ entails everything")
    ...
    check("preserving only-R₁ also 12 (D₆)", len(r1) == 12, f"={len(r1)}")
    check("each R₁-preserving one preserves all three", all(preserves(p) for p in r1))
...
def section_B():
    ...
    for n in (2, 3, 4):
        simplex_v, simplex_f = n + 1, n + 1            # vertices/facets of the simplex
        cross_v, cross_f = 2 * n, 2 ** n               # cross-polytope
        cube_v, cube_f = 2 ** n, 2 * n                 # cube
        check(f"n={n}: cube ({cube_v}v/{cube_f}f) ↔ cross-polytope ({cross_v}v/{cross_f}f) — counts swap places",
              (cube_v, cube_f) == (cross_f, cross_v))
        check(f"n={n}: simplex self-dual ({simplex_v}v/{simplex_f}f)", simplex_v == simplex_f)
...
def section_C():
    ...
    check("solution is unique: n=3", sols == [3], f"={sols}")
    check("6 vertices of the octahedron = |S₃|=6", 2 * 3 == factorial(3))

def section_D():
    print("\n[D] Builder/guardian: node where the sides coincide")
