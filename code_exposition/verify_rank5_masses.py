# -*- coding: utf-8 -*-
"""
verify_rank5_masses.py — HONEST test "rank 5 (golden φ) → mass hierarchy".
We compute real data (PDG), we don't fit. Discipline: φ-gold≈1.618 ≠ phase 2/9.
"""
import math
p=0; f=0
def ck(n,c):
...
# control: this (Q,δ) reproduces the real ratios
...
# ★ is 2/9 GOLDEN? we compare with golden candidates
gold_cand={"1/φ³":1/PHI**3,"2−φ":2-PHI,"1/φ²·":1/PHI**2-1/PHI**3}
best=min(abs(2/9-v) for v in gold_cand.values())
ck(f"★phase 2/9 — NOT golden (nearest golden candidate off {best/(2/9)*100:.1f}%>1%)", best/(2/9)>0.03)
ck("⟹ charged leptons: Koide(r3)+phase 2/9 — WITHOUT gold (φ≈1.618 does not participate)", True)

# ── φ-power test of ratios (does gold give powers?) ──
e_mue=math.log(mm/me)/math.log(PHI); e_taumu=math.log(mt/mm)/math.log(PHI)
ck(f"mass ratios ≠ powers of φ: log_φ(μ/e)={e_mue:.2f}, log_φ(τ/μ)={e_taumu:.2f} — NOT integers (fit)",
   abs(e_mue-round(e_mue))>0.05 and abs(e_taumu-round(e_taumu))>0.05)

# ── quarks: Koide NOT 2/3 (lepton-specific) ──
...
# ── WHERE gold is real: NEUTRINO mixing (A₅, tan θ₁₂=1/φ) — approximately ──
sin2_gold=1/(1+PHI**2)               # tan θ=1/φ ⟹ sin²θ=1/(1+φ²)
sin2_meas=0.307
off=abs(sin2_gold-sin2_meas)/sin2_meas
ck(f"★gold→neutrino: sin²θ₁₂(gold)={sin2_gold:.3f} vs meas {sin2_meas:.3f} — approximately (off {off*100:.0f}%, ◐)",
   0.05<off<0.20)
...
print()
verdict={
 "Koide Q=2/3 (angle of masses) — RANK 3 (σ½-facet), not 5": "● solid",
 "phase δ≈2/9 gives lepton ratios — but 2/9 is NOT gold, and NOT derived": "◐ recognition (0.07% coincidence)",
 "★gold φ≈1.618 does NOT give the hierarchy of charged leptons": "✗ hypothesis not confirmed",
 "gold is real → NEUTRINO mixing (A₅, tan θ₁₂=1/φ)": "◐ approximately (~10% off), not masses",
 "quarks Koide≠2/3; mass ratios ≠ powers of φ": "● (fit, not gold)",
