...
Hypothesis: the "missing radial coordinate" (the front after splitting off the underside,
doc 88 / ch. "Underside") is NOT structureless — on the ARCHIMEDEAN side it is
the CURVATURE AXIS (2,3,p) (doc 5), and on the p-ADIC side — the boundary of the Bruhat–Tits tree
(doc 19). Two sides of Ostrowski, stitched by ∏=1.

  [A] curvature axis δ(p)=(6−p)/6p: p<6 sphere(+), p=6 flat(0)=σ½, p>6 hyperbola(−);
  [B] σ½ = r=0 = flat center (p=6, δ=0, Ω_k=0) — start of the radial coordinate;
  [C] radial background ∇²φ=const = CONSTANT curvature (Poisson, homogeneous source);
  [D] p-adic side: Bruhat–Tits tree (p+1)-regular, the boundary is continuous;
  [E] two sides of Ostrowski stitched by ∏_v|x|_v=1: discrete(|·|_p, tree) × Archimedean(|·|∞, curvature).

Conclusion (honestly): the radial coordinate is STRUCTURED (= curvature, axis (2,3,p),
σ½=flat center), but the SPECIFIC curvature (Λ) = input (○), not derived. The wall holds.
...
def section_A():
    ...
    check("p=5 (gold): δ>0 — sphere (closed)", defect(5) > 0 and abs(defect(5) - 1 / 30) < 1e-12)
    check("p=6: δ=0 — FLAT (nucleus, σ½, Ω_k=0)", abs(defect(6)) < 1e-12)
    check("p=7 (Hurwitz): δ<0 — hyperbola (open, Λ-side)", defect(7) < 0 and abs(defect(7) + 1 / 42) < 1e-12)
...
def section_B():
    ...
    check("p=6 — the unique flat point (δ=0): radius r=0", abs(defect(6)) < 1e-12)
    check("|δ| grows on both sides of 6 (sphere 5←6→7 hyperbola)",
...
def section_C():
    ...
    check("κ=0 ⟹ ∇²φ=0 (flat=σ½); κ≠0 ⟹ curvature (sphere/hyperbola by sign)",
          abs(0.0) < 1e-12)
...
def section_D():
    ...
    check("boundary of the tree is continuous (ends→continuum): the finite does not fix a point",
          ends(2, 3) < ends(2, 10) < ends(2, 30))


def section_E():
    print("\n[E] two sides of Ostrowski stitched by ∏_v|x|_v=1")

    def is_prime(k):
        return k > 1 and all(k % i for i in range(2, int(k ** 0.5) + 1))

    def prod_one(n):
        """for integer n>1: |n|_∞ · ∏_p |n|_p = 1 (product formula)."""
        val = float(n)                              # |n|_∞
        m = n
    ...
    print("   ⟹ radial coordinate = Archimedean half = axis (2,3,p); σ½=flat center p=6")
...
def main():
    ...
    print("the specific curvature Λ = input ○, not derived — the wall of values holds.")
    print("=" * 64)
