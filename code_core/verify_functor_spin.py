...
Answer to the reviewer's objection "no morphism between levels": we build ONE explicit functor
and check it ON SEVERAL OPERATIONS (lift, κ, composition), not at a single coincidence point.

SOURCE 𝒟 (our combinatorics):
  objects   — hypercubes Q_n = graph on 𝔽₂ⁿ (vertices=bitstrings, edges=Hamming-1);
  morphisms — lift Λ: Q_n ↪ Q_{n+1}=Q_n □ K₂ (add axis); κ: x↦x+1ⁿ (complement).

TARGET 𝒫 (physics):
  objects   — n spin-½'s, Hamiltonian A_n=∑σ_x^(i) (transverse field / hops on the cube);
  morphisms — add spin; global flip X=∏σ_x.

FUNCTOR F: Q_n ↦ (ℂ²)^⊗n with A_n=∑σ_x; Λ ↦ ⊗(add spin); κ ↦ X=σ_x^⊗n.
KEY: A(Q_n) (hypercube adjacency matrix) is IDENTICALLY EQUAL to ∑σ_x^(i) — not a coincidence
of value but an equality of OPERATORS, whence the WHOLE structure carries over (spectrum, multiplicities,
κ-symmetry, lift). We check functoriality: F(Λ∘Λ)=F(Λ)∘F(Λ), F(κ²)=id etc.

HONEST BOUNDARY (answer to "bridge at a single point"): image of the functor = FREE spins (∑σ_x).
Mass/interaction/curvature require WEIGHTS on the edges (input, like so(4) from 1/r) — the bare
Q_n has none. The functor is reliable up to the free system; beyond that — open frontier [○].
"""
...
def single(op, i, n):
    """operator op on spin i of n (the rest — identity)."""
    return kron_list([op if j == i else I2 for j in range(n)])

def A_cube(n):
    """hypercube adjacency matrix Q_n = ∑ σ_x^(i) (bit-flip i = Hamming edge)."""
...
def section_A():
    ...
    for n in range(1, 7):
        ...
        # (1) spin operator IDENTICALLY = combinatorial adjacency matrix of the cube
...
def section_B():
    print("\n[B] lift Λ: Q_n→Q_{n+1}=Q_n□K₂  ↦  add spin (A_{n+1}=A_n⊗I+I⊗σ_x)")
    for n in range(1, 6):
        An = A_cube(n)
        An1_direct = A_cube(n + 1)
        An1_lift = np.kron(An, I2) + np.kron(np.eye(1 << n), SX)
        check(f"n={n}→{n+1}: F(Λ) = add spin (identity of operators)",
...
def section_C():
    ...
    for n in range(1, 6):
        ...
        check(f"n={n}: [A,κ]=0 — complement commutes (preserves levels)",
              np.allclose(A @ X, X @ A))
...
# ───────────────────────── E. σ½ as spectral center ─────────────────────────
def section_E():
    print("\n[E] σ½ = middle stratum k=n/2: max degeneracy C(n,n/2), massless center")
    for n in (2, 4, 6):
        A = A_cube(n)
        ev = np.round(np.linalg.eigvalsh(A)).astype(int)
        from collections import Counter
        c = Counter(ev.tolist())
        mid_deg = c[0]                     # eigenvalue 0 = middle k=n/2
        check(f"n={n}: middle λ=0 has MAX multiplicity C(n,n/2)={comb(n, n//2)}",
              mid_deg == comb(n, n // 2) and mid_deg == max(c.values()))
    print("   → center of the spectrum (λ=0, k=n/2) = σ½ = κ-fixed of the middle layer = massless middle")
...
def section_F():
    ...
    A = A_cube(n)
    ev = np.round(np.linalg.eigvalsh(A)).astype(int)
    # bare cube is vertex-transitive ⟹ middle is degenerate ⟹ NO mass gap
    gap_bare = sorted(set(ev.tolist()))
    has_zero = 0 in gap_bare
    check("bare Q_n: middle λ=0 present (massless, no gap) — free system",
          has_zero)
    # INTRODUCE a weight-imbalance on one axis (κ-break, doc 88): a gap opens, but weight g — INPUT
    g = 0.7
    A_weighted = sum((1.0 if i > 0 else 1.0 + g) * single(SX, i, n) for i in range(n))
    ev_w = np.linalg.eigvalsh(A_weighted)
    # check that the imbalance SHIFTS the structure (weight — free parameter, not from Q_n)
    moved = not np.allclose(sorted(ev.tolist()), sorted(np.round(ev_w).astype(int).tolist()))
    check("WEIGHTED cube (κ-break, weight g=INPUT): structure shifts — mass requires input",
          moved)
    print("   → functor is RELIABLE up to free spins (∑σ_x); mass/interaction/curvature")
    print("     = weights on edges = INPUT (like so(4) from 1/r) ⟹ bridge built AT A SINGLE POINT [○]")


def main():
    print("=" * 74)
    print("EXPLICIT FUNCTOR F: 𝒟(Q_n, Λ, κ) → 𝒫(spin system ∑σ_x)")
    print("functoriality check on OPERATIONS (lift/κ/composition), not at a point")
    print("=" * 74)
    ...
    print("       BUT the image = free system; rich physics = weights-input = open frontier [○].")
    print("=" * 74)
