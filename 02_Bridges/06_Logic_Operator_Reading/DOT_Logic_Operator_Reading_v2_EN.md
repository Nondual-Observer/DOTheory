# DOT: bridge — logico-operator reading of the operator tower

## Document status

This is a **bridge** document, not part of the strict core.

The strict core is constructed in `DO_Begin_III_fixed.md` up to and including §11. Native to the strict core: the polar layer §1, the automorphisms §2, the coordinate rank lift §3, rank 2 with the comparator §4, $X_{\mathrm{adm}}$ of rank 3 §5, the relation grammar $R_1,R_2,R_3$ §6, the octahedral skeleton $K_{2,2,2}$ §7, the chamber layer §8, the incidence package §9, cyclic transport §10, the periodization package $\mathcal P_T^{(3)}$ §11.

The document proposes an additional **reading** of already constructed carriers. The power-set carrier $Q_n$ is read as a Boolean algebra $\mathcal P(J_n)$. $X_{\mathrm{adm}}$ is read as a punctured Boolean carrier. The operator tower $\mathcal B_m\cong Q_{2^m}$ arises as iterated application of the power-set functor.

All objects of the document have the status `bridge / reading` unless explicitly marked otherwise. The strict-core names are preserved and carried over by cross-reference.
...
\]
acts on the operator tower $\mathcal B_m$. Its three nontrivial elements yield three series of natural subcarriers. Through this action the three appearances of the octahedron at rank 3, rank 4, and rank 8 become three instances of a single functorial law.
...
Strict core §1 constructs the polar layer $(P,R_P)$, $P=\{a,-a\}$. Strict core §3 gives the coordinate rank lift: the polarity $a$ receives the coordinate name $0$, the polarity $-a$ the name $1$. The rank-1 coordinate carrier is
\[
\mathbb D=\{0,1\}.
\]

`Cross-reference`: $\mathbb D$ is the coordinate reading of the polar layer $(P,R_P)$ from strict core §3.
...
This is the coordinate reading of the canonical swap $\tau$ from strict core §2.
...
In the rank-1 strict core neither `AND` nor `OR` is yet defined: they require a second coordinate digit (strict core §4). Therefore this section fixes exactly the coordinate involution, without any claim to functional completeness of the Boolean algebra.

The status of `NOT` in this bridge:
- `bridge name` for $\tau$ from strict core §2.
...
`Cross-reference`: $Q_n$ as the $n$-bit coordinate carrier is constructed in strict core §3 (rank 1), §4 (rank 2), §5 (rank 3).
...
`Cross-reference`: the shell decomposition of strict core §5.6 gives the same stratification for $J=J_3$.
...
`Cross-reference`: for $J=J_3$ and $d\in\{1,2,3\}$ this coincides with the relation grammar of strict core §6. Here $R_d^J$ is generalized functorially.
...
`Cross-reference`: for $J=J_3$ this is the complement involution of strict core §5.8.
...
Let $J_3=\{e_1,e_2,e_3\}$ be a three-element set with neutral coordinate labels. The names $e_i$ are chosen deliberately so as not to collide with the reserved names of the axial invariants of the semantic layer (the axial $D/F/C$ appear only in §§12–14 of the strict core, and are not used in this bridge).
...
`Cross-reference`: $X_3$ is exactly $X_{\mathrm{adm}}$ from strict core §5.5. This is one set in two readings: native — the admissible carrier of rank 3; bridge — the punctured Boolean carrier.
...
`Cross-reference`: these three complement pairs coincide with $\beta_1,\beta_2,\beta_3$ from the strict core §7 octahedral skeleton.
...
`Cross-reference`: this is exactly $R_1,R_2,R_3$ from strict core §6 with $J=J_3$, restricted to $X_3$.
...
Status: the Boolean operations exist on $Q_3$ as structure; on $X_3$ they serve as a `reading` from which the native relations $R_1,R_2,R_3$ are recovered.

## §3.7. Native and bridge on $X_3$

Native on $X_3$ (from the strict core):
...
All native objects of the strict core ($Q_n$, $X_{\mathrm{adm}}$, $R_d$, the complement pairs, the octahedral skeleton) do not depend on the names of the coordinates. They are natural subcarriers of $\mathcal Q$.
...
`Cross-reference`: this corresponds to the rank lift of strict core §3 ($P\to Q_2$) and the generalization §5 ($Q_n\to Q_{n+1}$).
...
Ranks $3,5,6,7$ do not have the form $\mathcal B_m$ for an integer $m$. They are native state carriers of DOT with their own grammar (for rank 3 — strict core §§5–11) and are not identified with operator carriers of standard arity.
...
`Cross-reference`: $Q_2$ is constructed in strict core §4. Here its operator reading as the space of unary Boolean operators is added.
...
`Discipline`: when referring to "rank 4" in a bridge document, use $\mathcal B_2^\circ$; when referring to "rank 4" in the strict-core methodology, use $Q_4^*$. These two scenes have one ambient $Q_4$, but different subcarriers.
...
`Cross-reference`: $K_{2,2,2}$ arose in strict core §7 as the octahedral skeleton on $X_{\mathrm{adm}}$. Here the same graph arose on $S_2^{(4)}\subset\mathcal B_2$. These are two different carriers with one and the same graph structure:
\[
...
Both carry $K_{2,2,2}$, but these are different native scenes: the first is the rank-3 state carrier; the second is the middle layer of the rank-4 operator carrier.
...
| Subgroup | Condition | Native series |
|---|---|---|
...
$\operatorname{Aff}_m$ is a native subcarrier of $\mathcal B_m$, singled out by the structure of $\mathbb F_2$-affinity on $\mathcal P(J_m)=\mathbb F_2^m$. It is not a subfunctor of $\mathcal B$ under bijections (a bijection $\sigma$ need not be $\mathbb F_2$-linear), but once a coordinate structure is chosen, $\operatorname{Aff}_m$ is well defined.
...
`Cross-reference`: for $m=2$ the analogous graph on $S_2^{(4)}$ (6 vertices minus 3 complement pairs) is the octahedral $K_{2,2,2}=K_{3\times 2}$ — the 1-skeleton of the 3-dimensional cross-polytope.
...
## §14.1. Input-complement pairs in $Q_3$
...
`Cross-reference`: $p_1,p_2,p_3$ coincide with the complement pairs $\beta_1,\beta_2,\beta_3$ from strict core §7. The pair $p_0$ is the removed total-pole pair $\{000,111\}$, absent from $X_{\mathrm{adm}}$.
...
The third octahedron in $\operatorname{Inv}_3\cap S_4^{(8)}$ structurally repeats the octahedral skeleton $X_{\mathrm{adm}}$:
...
# §15. The three appearances of the octahedron as instances of a single law
...
This is the abstract description of the octahedral skeleton.

## §15.2. The three instances

**Instance I — strict core, rank 3.**
\[
V=X_{\mathrm{adm}},
\qquad
\iota=\kappa_3|_{X_{\mathrm{adm}}}.
\]
Three orbits: $\beta_1,\beta_2,\beta_3$. The graph $R_{12}=R_1\cup R_2\cong K_{2,2,2}$. Native in strict core §7.

**Instance II — operator rank 4.**
\[
V=S_2^{(4)},
\qquad
...
| Instance | Carrier | Involution | Size | Source |
|---|---|---|---|---|
| I | $X_{\mathrm{adm}}\subset Q_3$ | $\kappa_3$ | 6 | strict core §7 |
| II | $S_2^{(4)}\subset\mathcal B_2$ | $C_{\mathrm{out}}$ | 6 | bridge §9 |
| III | $\operatorname{Inv}_3\cap S_4^{(8)}\subset\mathcal B_3$ | $C_{\mathrm{out}}$ | 6 | bridge §14 |
...
## §16.2. Operator ranks vs state ranks

The state ranks of DOT (`TNR_Stratification_Protocol`): $1,2,3,4,5,\ldots$ Each receives its own native carrier $Q_n^*$ and its own relation grammar.
...
- the methodological carrier of the strict core: $Q_4^*$, 15 elements P1–P15, native in `TNR_Stratification_Protocol` §3.5;
- the operator carrier of the bridge: $\mathcal B_2^\circ$, 14 elements as binary Boolean operators without constants, bridge §9.

These two subsets of $Q_4$ have the common part $Q_4\setminus\{0000,1111\}$ of 14 elements and differ by the inclusion of the point $1111$, which in the strict core falls into P15 (saturation), and in the bridge drops out as the second pole.

## §16.4. The octahedral law and the tower

The octahedral law manifests at three ranks of the tower:
\[
\text{rank 3 (state, strict)},
\qquad
\text{rank 4 (operator, bridge)},
\qquad
\text{rank 8 (inner subcarrier, bridge)}.
\]
...
# §17. The status of terms

## §17.1. Native in the strict core (used by cross-reference)

\[
P, R_P, \pi, I, \tau \quad(\text{§§1–2})
\]
\[
Q_n, X_{\mathrm{adm}}, R_1, R_2, R_3 \quad(\text{§§3–6})
\]
\[
\beta_i, K_{2,2,2}, \text{octahedral skeleton} \quad(\text{§7})
\]
\[
\text{chamber layer, incidence package} \quad(\text{§§8–9})
\]
\[
T, \mathcal P_T^{(3)} \quad(\text{§§10–11}).
\]

## §17.2. Bridge in this document
...
`Boolean operations as reading`:
\[
\cap,\cup,\neg,\oplus.
\]

`Operator tower carriers`:
\[
\mathcal B_J=\mathcal P(\mathcal P(J)),
\qquad
\mathcal B_m\cong Q_{2^m},
...
`Klein four-group`:
\[
G_{\mathrm{B}}=\langle C_{\mathrm{out}},C_{\mathrm{in}}\rangle.
\]

`Klein-invariant subcarriers`:
\[
\operatorname{Inv}_m,
\qquad
\operatorname{SD}_m,
...
`Octahedral instances` (repetitions of native §7 in the operator tower):
\[
S_2^{(4)},
\qquad
...
`Cross-polytope at rank 8`:
\[
K_{35\times 2}\text{ on }S_4^{(8)}.
\]

## §17.3. Not-yet bridge (beyond this document)

\[
\text{semantic axial layer }(D,F,C):\ \text{strict core §§12–14, not used here;}
\]
\[
\text{rank-4 methodology }P_1,\ldots,P_{15}:\ \text{strict core §§18–21, differs from }\mathcal B_2^\circ;
\]
\[
\text{Möbius carrier }p:E\to S^1:\ \text{bridge layer §22+, not native here;}
\]
\[
\text{literal Hopf/Borromean link}:\ \text{distant bridge layer.}
\]

## §17.4. Disclaimers

- The names $D, F, C$ are reserved for the semantic axial layer of the strict core. In this bridge the neutral $e_1,e_2,e_3$ are used.
- Octahedron and cross-polytope in this document mean only a finite graph isomorphism, not topological or geometric objects.
- The $K_{2,2,2}$ of rank 4 and the $K_{2,2,2}$ of rank 8 are two different carriers with the same graph structure; they cannot be identified as sets.
- $\operatorname{Aff}_3^\circ$ and $\mathcal B_2^\circ$ are both 14-element and in coordinate notation coincide as subsets of $Q_4$, but they realize different roles: the first is an affine subcarrier inside $\mathcal B_3$, the second is the operator carrier of rank 4 as such.
...
**First level.** The power-set carrier $Q_J=\mathcal P(J)$ with the Hamming relation $R_d^J(A,B)\iff|A\triangle B|=d$ functorially generalizes the relation grammar of strict core §6 to an arbitrary finite $J$. At rank 3 this functor gives the punctured Boolean carrier $X_3\cong X_{\mathrm{adm}}$, and the Boolean operations $\cap,\cup,\neg,\oplus$ arise as a reading from which the native $R_1,R_2,R_3$ are recovered.
...
**The octahedral law.** $K_{2,2,2}$ arises as the graph "$V$ with an involution of order 2 having 3 fixed pairs" whenever such a structure is given. The three instances — strict core §7 ($X_{\mathrm{adm}}$ with $\kappa_3$), bridge §9 ($S_2^{(4)}$ with $C_{\mathrm{out}}$), bridge §14 ($\operatorname{Inv}_3\cap S_4^{(8)}$ with $C_{\mathrm{out}}$) — are three realizations of a single functorial law. The bijection between the third octahedron and the rank-3 axes is written out via the partitions of the 4 input-complement orbits $\{p_0,p_1,p_2,p_3\}$ into 2+2 with the partner for $p_0=\{000,111\}$ fixed.

**Next layers.** The native semantic axial layer of strict core §§12–14 (vertex/opposite-edge reading, $X_{\mathrm{adm}}\cong I_3\times\{-,+\}$), the native rank-4 methodology of strict core §§18–21 (P1–P15 in $Q_4^*$), the bridge Möbius §22+ and the literal Hopf/Borromean remain outside this document. The operator tower provides an additional language, but does not supplant the strict core: the native structures of the strict core continue to be the foundation, and the operator tower attaches to them as a functorial extension via the Klein group of natural automorphisms.
