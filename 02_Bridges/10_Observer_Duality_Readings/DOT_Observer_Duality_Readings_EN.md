...
## Reading 6. Weyl and Cartan: the observer-`i` as the root of the polarity; the Möbius band **(r3 + all-rank spectrally)**

The complement $\kappa$ is the **Weyl involution** of the $\mathfrak{sl}_2$ representation (Volume 7 §2.3): it swaps raising and lowering ($\partial\leftrightarrow\delta$) and negates the weight ($\kappa H\kappa=-H$). The continuous shadow (Reading 5) adds a **second** involutive face — the **Cartan decomposition** of the algebra into a compact part (rotation $so(2)$, closed orbits, discrete spectrum) and a non-compact part (boosts, open orbits, continuous spectrum); it is set by the **Cartan involution** $\theta(X)=-X^{\mathsf T}$. The Weyl $\kappa$ and the Cartan $\theta$ are **not the same map, but differ by exactly the observer**:
$$\theta=\operatorname{Ad}\!\big(e^{\,i\pi H/2}\big)\circ\kappa,\qquad e^{\,i\pi H/2}=\operatorname{diag}(i,-i).$$
The factor $e^{i\pi H/2}$ is a **quarter-turn by the imaginary unit** — the very $i$ through which the atom lifts $1D\to2D$ (the observer, Volume 0). It sorts discrete and continuous **oppositely**: $\kappa$ fixes a continuous direction ($e+f$, real eigenvalues $\pm1$, a boost), $\theta$ fixes a discrete one ($e-f$, imaginary $\pm i$, a rotation). So discrete and continuous are two sides of one transition, and the operator between them is the observer $i$ (= the quarter Wick rotation $s\leftrightarrow it$ of Reading 5).

The powers of $i$ form a $\mathbb Z/4$ tower whose levels are already-established corpus facts: $i^1$ (quarter) = the Weyl↔Cartan = the discrete↔continuous bridge; $i^2=-1$ (half) = the $e^{i\pi}=-1$ of Reading 5 = the spectral face of $T^3=\kappa$, and on three qubits (Reading 4) the spinor sign of the double cover $SU(2)\to SO(3)$ = the **Möbius band** (a half-turn that reverses orientation); $i^4=1$ (full) = return. Hence a deeper level of the through-thread: the central polarity $\pm1$ (the whole table below) is $i^2$ — the **square** of the observer; the observer-$i$ is its **square root** (the quarter-turn), and it is what carries the discrete/continuous transition. The Möbius band is the geometric name of the $i^2$ level: the half-return after which the scene comes back with reversed orientation — exactly $T^3=\kappa$ with $\det(\kappa)=-1$ (the screw's improperness, Reading 3), the $SU(2)$ spinor ($2\pi\mapsto-1$, $4\pi\mapsto1$, Reading 4). The same $i$-symmetry recurs wherever discrete passes into continuous: Euler↔Laplace ($s\leftrightarrow it$), compact $su(2)$↔non-compact $sl(2,\mathbb R)$, thermal↔quantum ($e^{-\beta H}\leftrightarrow e^{-itH}$), the cycle $C_6$↔torus (Volume 4), DC mode↔Nyquist — one pattern, "multiply by $i$" = the discrete/continuous seam = the observer.

*Precision note (forcedness > beauty): the corpus's $\kappa$ is the **Weyl**, not Cartan, involution; $H$ is the **Cartan grading**, not an involution. The Cartan involution $\theta$ is a separate (kindred) face, and $\theta=\operatorname{Ad}(e^{i\pi H/2})\circ\kappa$ is an elementary $\mathfrak{sl}_2$ computation ($(e^{i\pi H/2})^2=-I$). A reading is a translation, not a new algebra. Continuous-side details: `Discrete_Continuous_From_One_Triple_RU.md`.*
